...
pub const DUST_THRESHOLD: I80F48 = I80F48!(0.000001); // TODO make this part of LyraeGroup state
...
pub struct LyraeGroup {
    ...
    pub lyrae_cache: Pubkey,
    pub valid_interval: u64,

    // insurance vault is funded by the Lyrae DAO with USDC and can be withdrawn by the DAO
    pub insurance_vault: Pubkey,
    pub srm_vault: Pubkey,
    pub msrm_vault: Pubkey,
    pub fees_vault: Pubkey,

    pub max_lyrae_accounts: u32, // limits maximum number of LyraeAccounts.v1 (closeable) accounts
    pub num_lyrae_accounts: u32, // number of LyraeAccounts.v1

    pub ref_surcharge_centibps: u32, // 100
    pub ref_share_centibps: u32,     // 80 (must be less than surcharge)
    pub ref_lyr_required: u64,
...
impl LyraeGroup {
    pub fn load_mut_checked<'a>(
        ...
        program_id: &Pubkey,
    ) -> LyraeResult<RefMut<'a, Self>> {
        check_eq!(account.owner, program_id, LyraeErrorCode::InvalidOwner)?;

        let lyrae_group: RefMut<'a, Self> = Self::load_mut(account)?;
        check!(
            lyrae_group.meta_data.is_initialized,
            LyraeErrorCode::InvalidAccount
        )?;
        check_eq!(
            lyrae_group.meta_data.data_type,
            DataType::LyraeGroup as u8,
            LyraeErrorCode::InvalidAccount
        )?;
    ...
    pub fn load_checked<'a>(
        account: &'a AccountInfo,
        program_id: &Pubkey,
    ) -> LyraeResult<Ref<'a, Self>> {
        check_eq!(account.owner, program_id, LyraeErrorCode::InvalidOwner)?;

        let lyrae_group: Ref<'a, Self> = Self::load(account)?;
        check!(
            lyrae_group.meta_data.is_initialized,
            LyraeErrorCode::InvalidAccount
        )?;
    ...
    pub fn find_root_bank_index(&self, root_bank_pk: &Pubkey) -> Option<usize> {
        ...
        self.tokens
            .iter()
            .position(|token_info| &token_info.root_bank == root_bank_pk)
    }
    pub fn find_token_index(&self, mint_pk: &Pubkey) -> Option<usize> {
        self.tokens
            .iter()
            .position(|token_info| &token_info.mint == mint_pk)
    }
...
impl RootBank {
    pub fn load_and_init<'a>(
        ...
        check!(!root_bank.meta_data.is_initialized, LyraeErrorCode::Default)?;
    ...
    pub fn set_rate_params(
        ...
        check!(optimal_rate >= ZERO_I80F48, LyraeErrorCode::InvalidParam)?;
        check!(max_rate >= ZERO_I80F48, LyraeErrorCode::InvalidParam)?;
    ...
    pub fn load_mut_checked<'a>(
        ...
        check!(
            root_bank.meta_data.is_initialized,
            LyraeErrorCode::InvalidAccount
        )?;
        check_eq!(
    ...
    pub fn update_index(
        ...
        let utilization = native_borrows
            .checked_div(native_deposits)
        ...
        let borrow_interest: I80F48 = interest_rate
            .checked_mul(I80F48::from_num(now_ts - self.last_updated))
            .unwrap();
    ...
    pub fn socialize_loss(
        ...
        program_id: &Pubkey,
        token_index: usize,
        lyrae_cache: &mut LyraeCache,
        bankrupt_account: &mut LyraeAccount,
        ...
        for i in 0..self.num_node_banks {
            check!(
                node_bank_ais[i].key == &self.node_banks[i],
        ...
        lyrae_cache.root_bank_cache[token_index].deposit_index = self.deposit_index;
...
impl NodeBank {
    pub fn load_and_init<'a>(
        ...
        check!(!node_bank.meta_data.is_initialized, LyraeErrorCode::Default)?;
    ...
    pub fn load_mut_checked<'a>(
        ...
        check!(
            node_bank.meta_data.is_initialized,
            LyraeErrorCode::InvalidAccount
        )?;
        check_eq!(
    ...
    pub fn checked_add_borrow(&mut self, v: I80F48) -> LyraeResult<()> {
        Ok(self.borrows = self.borrows.checked_add(v).ok_or(throw!())?)
    }
    pub fn checked_sub_borrow(&mut self, v: I80F48) -> LyraeResult<()> {
        Ok(self.borrows = self.borrows.checked_sub(v).ok_or(throw!())?)
    }
    pub fn checked_add_deposit(&mut self, v: I80F48) -> LyraeResult<()> {
        Ok(self.deposits = self.deposits.checked_add(v).ok_or(throw!())?)
    }
    pub fn checked_sub_deposit(&mut self, v: I80F48) -> LyraeResult<()> {
...
impl PriceCache {
    pub fn check_valid(&self, lyrae_group: &LyraeGroup, now_ts: u64) -> LyraeResult<()> {
        check!(
            self.last_update >= now_ts - lyrae_group.valid_interval,
            LyraeErrorCode::InvalidPriceCache
        )
    }
}
...
impl RootBankCache {
    pub fn check_valid(&self, lyrae_group: &LyraeGroup, now_ts: u64) -> LyraeResult<()> {
        check!(
            self.last_update >= now_ts - (lyrae_group.valid_interval * 2),
            LyraeErrorCode::InvalidRootBankCache
        )
    }
}
...
impl PerpMarketCache {
    pub fn check_valid(&self, lyrae_group: &LyraeGroup, now_ts: u64) -> LyraeResult<()> {
        check!(
            self.last_update >= now_ts - lyrae_group.valid_interval,
            LyraeErrorCode::InvalidPerpMarketCache
        )
    }
}
...
impl LyraeCache {
    pub fn load_mut_checked<'a>(
        ...
        // lyrae account must be rent exempt to even be initialized
        check_eq!(account.owner, program_id, LyraeErrorCode::InvalidOwner)?;
        let lyrae_cache = Self::load_mut(account)?;

        check_eq!(
            lyrae_cache.meta_data.data_type,
            DataType::LyraeCache as u8,
            LyraeErrorCode::Default
        )?;
        check!(
            lyrae_cache.meta_data.is_initialized,
            LyraeErrorCode::Default
        )?;
        check_eq!(
            &lyrae_group.lyrae_cache,
            account.key,
            LyraeErrorCode::Default
        )?;
    ...
    pub fn load_checked<'a>(
        ...
        let lyrae_cache = Self::load(account)?;
    ...
    pub fn check_valid(
        ...
        for i in 0..lyrae_group.num_oracles {
            if active_assets.spot[i] || active_assets.perps[i] {
                self.price_cache[i].check_valid(&lyrae_group, now_ts)?;
            }

            if active_assets.spot[i] {
                self.root_bank_cache[i].check_valid(&lyrae_group, now_ts)?;
            }

            if active_assets.perps[i] {
                self.perp_market_cache[i].check_valid(&lyrae_group, now_ts)?;
            }
        }
        self.root_bank_cache[QUOTE_INDEX].check_valid(&lyrae_group, now_ts)
    }
...
impl UserActiveAssets {
    pub fn new(
        ...
        for i in 0..lyrae_group.num_oracles {
            spot[i] = !lyrae_group.spot_markets[i].is_empty()
                && (lyrae_account.in_margin_basket[i]
                    || !lyrae_account.deposits[i].is_zero()
                    || !lyrae_account.borrows[i].is_zero());

            perps[i] = !lyrae_group.perp_markets[i].is_empty()
                && lyrae_account.perp_accounts[i].is_active();
        }
...
impl HealthCache {
    ...
    pub fn init_vals(
        ...
        self.quote = lyrae_account.get_net(&lyrae_cache.root_bank_cache[QUOTE_INDEX], QUOTE_INDEX);
        for i in 0..lyrae_group.num_oracles {
            if self.active_assets.spot[i] {
                self.spot[i] = lyrae_account.get_spot_val(
                    &lyrae_cache.root_bank_cache[i],
                    lyrae_cache.price_cache[i].price,
                    i,
            ...
            if self.active_assets.perps[i] {
                self.perp[i] = lyrae_account.perp_accounts[i].get_val(
                    &lyrae_group.perp_markets[i],
                    &lyrae_cache.perp_market_cache[i],
                    lyrae_cache.price_cache[i].price,
                )?;
            }
        }
    ...
    pub fn get_health(&mut self, lyrae_group: &LyraeGroup, health_type: HealthType) -> I80F48 {
        ...
        match self.health[health_index] {
            None => {
                ...
                for i in 0..lyrae_group.num_oracles {
                    let spot_market_info = &lyrae_group.spot_markets[i];
                    let perp_market_info = &lyrae_group.perp_markets[i];
    ...
    pub fn update_quote(&mut self, lyrae_cache: &LyraeCache, lyrae_account: &LyraeAccount) {
        let quote = lyrae_account.get_net(&lyrae_cache.root_bank_cache[QUOTE_INDEX], QUOTE_INDEX);
    ...
    pub fn update_spot_val(
        ...
        let (base, quote) = lyrae_account.get_spot_val(
            &lyrae_cache.root_bank_cache[market_index],
            lyrae_cache.price_cache[market_index].price,
            market_index,
        ...
        for i in 0..NUM_HEALTHS {
            if let Some(h) = self.health[i] {
                ...
                let smi = &lyrae_group.spot_markets[market_index];
    ...
    pub fn update_token_val(
        ...
        if token_index == QUOTE_INDEX {
            Ok(self.update_quote(lyrae_cache, lyrae_account))
        } else {
    ...
    pub fn get_health_after_sim_perp(
        ...
        let (base, quote) = lyrae_account.perp_accounts[market_index].sim_get_val(
            info,
        ...
        let taker_fees = if taker_quote != 0 {
            ...
            if let Some(lyr_index) = lyrae_group.find_token_index(&lyr_token::id()) {
                let lyr_cache = &lyrae_cache.root_bank_cache[lyr_index];
                let lyr_deposits = lyrae_account.get_native_deposit(lyr_cache, lyr_index)?;
                let ref_lyr_req = I80F48::from_num(lyrae_group.ref_lyr_required);
                if lyr_deposits < ref_lyr_req {
                    market_fees += (I80F48::from_num(lyrae_group.ref_surcharge_centibps)
                        / CENTIBPS_PER_UNIT)
                        * taker_quote_native;
                }
            }
    ...
    pub fn update_perp_val(
        ...
        let (base, quote) = lyrae_account.perp_accounts[market_index].get_val(
...
pub struct LyraeAccount {
    ...
    // Alternative authority/signer of transactions for a lyrae account
    pub delegate: Pubkey,
...
impl LyraeAccount {
    pub fn load_mut_checked<'a>(
        ...
        // lyrae account must be rent exempt to even be initialized
        check_eq!(account.owner, program_id, LyraeErrorCode::InvalidOwner)?;
        let lyrae_account: RefMut<'a, Self> = Self::load_mut(account)?;

        check_eq!(
            lyrae_account.meta_data.data_type,
            DataType::LyraeAccount as u8,
            LyraeErrorCode::InvalidAccountState
        )?;
        check!(
            lyrae_account.meta_data.is_initialized,
            LyraeErrorCode::InvalidAccountState
        )?;
        check_eq!(
            &lyrae_account.lyrae_group,
            lyrae_group_pk,
            LyraeErrorCode::InvalidAccount
        )?;
    ...
    pub fn load_checked<'a>(
        ...
        let lyrae_account = Self::load(account)?;
    ...
    pub fn get_native_deposit(
        ...
        self.deposits[token_i]
            .checked_mul(root_bank_cache.deposit_index)
            .ok_or(math_err!())
    }
    pub fn get_native_borrow(
        ...
        self.borrows[token_i]
            .checked_mul(root_bank_cache.borrow_index)
            .ok_or(math_err!())
    }

    // TODO - Add unchecked versions to be used when we're confident
    // TODO OPT - remove negative and zero checks if we're confident
    pub fn checked_add_borrow(&mut self, token_i: usize, v: I80F48) -> LyraeResult<()> {
    ...
    pub fn checked_sub_borrow(&mut self, token_i: usize, v: I80F48) -> LyraeResult<()> {
        self.borrows[token_i] = self.borrows[token_i].checked_sub(v).ok_or(math_err!())?;

        check!(
            !self.borrows[token_i].is_negative(),
            LyraeErrorCode::MathError
        )?;
        check!(
            self.borrows[token_i].is_zero() || self.deposits[token_i].is_zero(),
    ...
    pub fn checked_add_deposit(&mut self, token_i: usize, v: I80F48) -> LyraeResult<()> {
    ...
    pub fn checked_sub_deposit(&mut self, token_i: usize, v: I80F48) -> LyraeResult<()> {
        self.deposits[token_i] = self.deposits[token_i].checked_sub(v).ok_or(math_err!())?;

        check!(
            !self.deposits[token_i].is_negative(),
            LyraeErrorCode::MathError
        )?;
        check!(
            self.borrows[token_i].is_zero() || self.deposits[token_i].is_zero(),
    ...
    fn get_spot_val<T: Deref<Target = serum_dex::state::OpenOrders>>(
        ...
        if !self.in_margin_basket[market_index] || open_orders.is_none() {
            ...
            if bids_base_net.abs() > asks_base_net.abs() {
                ...
                Ok((
                    asks_base_net * price,
                    base_locked * price + quote_free + quote_locked,
                ))
            }
        }
    }
    ...
    pub fn add_to_basket(&mut self, market_index: usize) -> LyraeResult<()> {
        if self.num_in_margin_basket == MAX_NUM_IN_MARGIN_BASKET {
            check!(
                self.in_margin_basket[market_index],
                LyraeErrorCode::MarginBasketFull
            )
        } else {
    ...
    pub fn check_enter_bankruptcy(
        ...
        //          -> should store DUST_THRESHOLD per token in LyraeGroup but this is breaking
    ...
    /// Note entering bankruptcy is calculated differently from exiting bankruptcy because of
    /// possible rounding issues and dust
    pub fn check_exit_bankruptcy(&self, lyrae_group: &LyraeGroup) -> bool {
    ...
    pub fn checked_unpack_open_orders<'a, 'b>(
        ...
        lyrae_group: &LyraeGroup,
        packed_open_orders_ais: &'a [AccountInfo<'b>],
    ) -> LyraeResult<Vec<Option<&'a AccountInfo<'b>>>> {
    ...
    pub fn add_order(&mut self, market_index: usize, side: Side, order: &LeafNode) -> LyraeResult {
        match side {
            Side::Bid => {
    ...
    ///
    pub fn remove_order(&mut self, slot: usize, quantity: i64) -> LyraeResult<()> {
        check!(
    ...
    pub fn max_withdrawable(
        ...
        if health.is_positive() && self.deposits[token_index].is_positive() {
            let price = lyrae_cache.get_price(token_index);
            let init_asset_weight = group.get_token_asset_weight(token_index, HealthType::Init);
            let health_implied = (health / (price * init_asset_weight))
                .checked_floor()
                .unwrap();
            let native_deposits: I80F48 = self
                .get_native_deposit(&lyrae_cache.root_bank_cache[token_index], token_index)?
                .checked_floor()
                .unwrap();
    ...
    pub fn get_complete_base_pos(
        ...
        for event in event_queue.iter() {
            if EventType::try_from(event.event_type).map_err(|_| throw!())? == EventType::Fill {
                ...
                if &fill.maker == lyrae_account_pk {
...
impl PerpAccount {
    ...
    fn convert_points(
        ...
        let points_in_period = I80F48::from_num(lmi.lyr_left)
            .checked_div(lmi.rate)
            .unwrap();

        if points >= points_in_period {
            self.lyr_accrued += lmi.lyr_left;
            ...
            lmi.lyr_left = lmi.lyr_per_period;
        }

        let lyr_earned = points
            .checked_mul(lmi.rate)
            .unwrap()
            .to_num::<u64>()
            .min(lmi.lyr_per_period); // limit lyr payout to max lyr in a period

        self.lyr_accrued += lyr_earned;
        lmi.lyr_left -= lyr_earned;
    }
    ...
    pub fn apply_size_incentives(
        ...
        if lmi.rate == 0 || lmi.lyr_per_period == 0 {
            return Ok(());
        }
    ...
    pub fn apply_price_incentives(
        ...
        check!(!points.is_negative(), LyraeErrorCode::MathError)?;
        self.convert_points(lmi, time_final, points);
        Ok(())
    }
...
pub struct LiquidityMiningInfo {
    /// Used to convert liquidity points to LYR
    pub rate: I80F48,

    pub max_depth_bps: I80F48, // instead of max depth bps, this should be max num contracts

    /// start timestamp of current liquidity incentive period; gets updated when lyr_left goes to 0
    pub period_start: u64,

    /// Target time length of a period in seconds
    pub target_period_length: u64,

    /// Paper LYR left for this period
    pub lyr_left: u64,

    /// Total amount of LYR allocated for current period
    pub lyr_per_period: u64,
}
...
pub struct PerpMarket {
    ...
    // lyr_vault holds lyrae tokens to be disbursed as liquidity incentives for this perp market
    pub lyr_vault: Pubkey,
}
...
impl PerpMarket {
    pub fn load_and_init<'a>(
        ...
        check!(account.owner == program_id, LyraeErrorCode::InvalidOwner)?;
        check!(
            rent.is_exempt(account.lamports(), size_of::<Self>()),
            LyraeErrorCode::AccountNotRentExempt
        )?;
        check!(!state.meta_data.is_initialized, LyraeErrorCode::Default)?;
        ...
        state.lyrae_group = *lyrae_group_ai.key;
        ...
        let vault = Account::unpack(&lyr_vault_ai.try_borrow_data()?)?;
        check!(
            vault.owner == lyrae_group.signer_key,
        ...
        check!(vault.mint == lyr_token::ID, LyraeErrorCode::InvalidVault)?;
        check!(
            lyr_vault_ai.owner == &spl_token::ID,
            LyraeErrorCode::InvalidOwner
        )?;
        state.lyr_vault = *lyr_vault_ai.key;
    ...
    pub fn load_checked<'a>(
        ...
        check!(
            lyrae_group_pk == &state.lyrae_group,
            LyraeErrorCode::Default
        )?;
        Ok(state)
    }
...
pub fn load_market_state<'a>(
    ...
    program_id: &Pubkey,
) -> LyraeResult<RefMut<'a, serum_dex::state::MarketState>> {
    check_eq!(
...
fn strip_dex_padding<'a>(acc: &'a AccountInfo) -> LyraeResult<Ref<'a, [u8]>> {
    check!(acc.data_len() >= 12, LyraeErrorCode::Default)?;
...
pub fn load_open_orders_accounts<'a>(
    ...
    accs.iter()
        .map(|ai_opt| {
            Ok(if let Some(ai) = ai_opt {
                Some(load_open_orders(ai)?)
            } else {
                None
            })
        })
...
pub fn check_open_orders(
    ...
    check_eq!(
        identity(open_orders.owner),
        owner.to_aligned_bytes(),
        LyraeErrorCode::Default
    )?;
    check!(acc.owner == dex_program_id, LyraeErrorCode::InvalidOwner)
}

fn strip_dex_padding_mut<'a>(acc: &'a AccountInfo) -> LyraeResult<RefMut<'a, [u8]>> {
    check!(acc.data_len() >= 12, LyraeErrorCode::Default)?;
...
) -> LyraeResult<(RefMut<'a, H>, RefMut<'a, [D]>)> {
...
pub fn load_bids_mut<'a>(
    ...
    bids: &'a AccountInfo,
) -> LyraeResult<RefMut<'a, serum_dex::critbit::Slab>> {
    check_eq!(
        &bids.key.to_aligned_bytes(),
        &identity(sm.bids),
        LyraeErrorCode::Default
    )?;
...
impl AdvancedOrders {
    pub fn init(account: &AccountInfo, program_id: &Pubkey, rent: &Rent) -> LyraeResult<()> {
    ...
    pub fn load_mut_checked<'a>(
        ...
        check!(
            &lyrae_account.advanced_orders_key == account.key,
            LyraeErrorCode::InvalidAccount
        )?;
        Ok(state)
    }
...
/// Store the referrer's lyrae account
#[derive(Copy, Clone, Pod, Loadable)]
#[repr(C)]
...
impl ReferrerMemory {
    pub fn init(
        account: &AccountInfo,
        program_id: &Pubkey,
        referrer_lyrae_account_ai: &AccountInfo,
    ) -> LyraeResult {
        let mut state: RefMut<Self> = Self::load_mut(account)?;
        check!(account.owner == program_id, LyraeErrorCode::InvalidOwner)?;
        check!(
        ...
        state.referrer_lyrae_account = *referrer_lyrae_account_ai.key;
