...
impl<'a, H: QueueHeader> Queue<'a, H> {
    ...
    pub fn load_mut(account: &'a AccountInfo) -> LyraeResult<Self> {
    ...
    pub fn revert_pushes(&mut self, desired_len: usize) -> LyraeResult<()> {
        check!(desired_len <= self.header.count(), LyraeErrorCode::Default)?;
...
impl FillEvent {
    pub fn new(
        ...
        Self {
            ...
            maker_out,
            version,
            market_fees_applied: true, // Since lyrae v3.3.5, market fees are adjusted at matching time
            padding: [0u8; 2],
            timestamp,
            seq_num,
            maker,
    ...
    pub fn to_fill_log(&self, lyrae_group: Pubkey, market_index: usize) -> FillLog {
        FillLog {
            lyrae_group,
