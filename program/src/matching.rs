...
use lyrae_logs::{lyrae_emit, ReferralFeeAccrualLog};
use lyrae_macro::{Loadable, Pod};
...
use crate::state::{
    DataType, LyraeAccount, LyraeCache, LyraeGroup, MetaData, PerpMarket, PerpMarketCache,
...
impl BookSide {
    ...
    pub fn load_mut_checked<'a>(
        ...
        match DataType::try_from(state.meta_data.data_type).unwrap() {
            DataType::Bids => check!(account.key == &perp_market.bids, LyraeErrorCode::Default)?,
            DataType::Asks => check!(account.key == &perp_market.asks, LyraeErrorCode::Default)?,
            _ => return Err(throw!()),
        }
    ...
    fn insert(&mut self, val: &AnyNode) -> LyraeResult<NodeHandle> {
    ...
    pub fn insert_leaf(
        &mut self,
        new_leaf: &LeafNode,
    ) -> LyraeResult<(NodeHandle, Option<LeafNode>)> {
...
impl<'a> Book<'a> {
    pub fn load_checked(
        ...
        check!(bids_ai.key == &perp_market.bids, LyraeErrorCode::InvalidAccount)?;
        check!(asks_ai.key == &perp_market.asks, LyraeErrorCode::InvalidAccount)?;
        Ok(Self {
    ...
    pub fn new_order(
        ...
        client_order_id: u64,
        now_ts: u64,
        referrer_lyrae_account_ai: Option<&AccountInfo>,
    ) -> LyraeResult {
        match side {
    ...
    fn new_bid(
        ...
        let mut referrer_lyrae_account_opt = None;
        ...
        while rem_quantity > 0 {
            ...
            if price < best_ask_price {
                ...
                return Ok(()); // return silently to not fail other instructions in tx
                               // return Err(throw_err!(LyraeErrorCode::PostOnly));
            }
            ...
            lyrae_account.perp_accounts[market_index].add_taker_trade(match_quantity, -match_quote);
        ...
        if rem_quantity > 0 && post_allowed {
            if self.bids.is_full() {
                ...
                check!(price > min_bid.price(), LyraeErrorCode::OutOfSpace)?;
            ...
            let owner_slot = lyrae_account
                .next_order_slot()
                .ok_or(throw_err!(LyraeErrorCode::TooManyOpenOrders))?;
            ...
            lyrae_account.add_order(market_index, Side::Bid, &new_bid)?;
        }
    ...
    pub fn new_ask(
        ...
        if rem_quantity > 0 && post_allowed {
            if self.asks.is_full() {
                ...
                check!(price < max_ask.price(), LyraeErrorCode::OutOfSpace)?;
                let event = OutEvent::new(
                    Side::Ask,
                    max_ask.owner_slot,
                    now_ts,
            ...
            lyrae_account.add_order(market_index, Side::Ask, &new_ask)?;
        }
    ...
    pub fn cancel_order(&mut self, order_id: i128, side: Side) -> LyraeResult<LeafNode> {
        match side {
            Side::Bid => {
                self.bids.remove_by_key(order_id).ok_or(throw_err!(LyraeErrorCode::InvalidOrderId))
            }
            Side::Ask => {
                self.asks.remove_by_key(order_id).ok_or(throw_err!(LyraeErrorCode::InvalidOrderId))
            }
        }
    }
    ...
    pub fn cancel_all(
        ...
        for i in 0..MAX_PERP_OPEN_ORDERS {
            if lyrae_account.order_market[i] != market_index {
                // means slot is free or belongs to different perp market
                continue;
            }
            let order_id = lyrae_account.orders[i];
            match self.cancel_order(order_id, lyrae_account.order_side[i]) {
                Ok(order) => {
    ...
    pub fn cancel_all_side_with_size_incentives(
        ...
        for i in 0..MAX_PERP_OPEN_ORDERS {
            if lyrae_account.order_market[i] == market_index_u8
                && lyrae_account.order_side[i] == side
            {
                all_order_ids.push(lyrae_account.orders[i]);
                keys.push(lyrae_account.orders[i])
            }
        }
        match side {
    ...
    pub fn cancel_all_with_size_incentives(
        ...
        for i in 0..MAX_PERP_OPEN_ORDERS {
            ...
            all_order_ids.push(lyrae_account.orders[i]);
            match lyrae_account.order_side[i] {
                Side::Bid => bids_keys.push(lyrae_account.orders[i]),
                Side::Ask => asks_keys.push(lyrae_account.orders[i]),
            }
        }
    ...
    /// Cancel all the orders for LyraeAccount for this PerpMarket up to `limit`
    ...
    pub fn cancel_all_with_price_incentives(
        ...
        for i in 0..MAX_PERP_OPEN_ORDERS {
            if lyrae_account.order_market[i] != market_index as u8 {
                // means slot is free or belongs to different perp market
                continue;
            }
            let order_id = lyrae_account.orders[i];
            let order_side = lyrae_account.order_side[i];
...
fn determine_ref_vars<'a>(
    ...
    referrer_lyrae_account_ai: Option<&'a AccountInfo>,
    now_ts: u64,
) -> LyraeResult<(I80F48, Option<RefMut<'a, LyraeAccount>>)> {
    let lyr_index = match lyrae_group.find_token_index(&lyr_token::id()) {
    ...
    // If the user's LYR deposit is non-zero then the rootbank cache will be checked already in `place_perp_order`.
    ...
    } else if let Some(referrer_lyrae_account_ai) = referrer_lyrae_account_ai {
        // If referrer_lyrae_account is invalid, just treat it as if it doesn't exist
        if let Ok(referrer_lyrae_account) =
        LyraeAccount::load_mut_checked(referrer_lyrae_account_ai, program_id, lyrae_group_pk)
        {
            // Need to check if it's valid because user may not have lyr in active assets
            lyr_cache.check_valid(lyrae_group, now_ts)?;
            let ref_lyr_deposits =
                referrer_lyrae_account.get_native_deposit(lyr_cache, lyr_index)?;

            if !referrer_lyrae_account.is_bankrupt
                && !referrer_lyrae_account.being_liquidated
                && ref_lyr_deposits >= ref_lyr_req
            {
                return Ok((
                    I80F48::from_num(lyrae_group.ref_share_centibps) / CENTIBPS_PER_UNIT,
                    Some(referrer_lyrae_account),
                ));
            }
        }
    }
    Ok((I80F48::from_num(lyrae_group.ref_surcharge_centibps) / CENTIBPS_PER_UNIT, None))
}
...
fn apply_fees(
    ...
    lyrae_account_pk: &Pubkey,
    market_index: usize,
    referrer_lyrae_account_opt: Option<RefMut<LyraeAccount>>,
    referrer_lyrae_account_ai: Option<&AccountInfo>,
    ...
    if ref_fee_rate > ZERO_I80F48 {
        ...
        // if ref lyrae account is some, then we send some fees over
        if let Some(mut referrer_lyrae_account) = referrer_lyrae_account_opt {
            lyrae_account.perp_accounts[market_index].transfer_quote_position(
                &mut referrer_lyrae_account.perp_accounts[market_index],
                ref_fees,
            );
            emit_perp_balances(
                referrer_lyrae_account.lyrae_group,
                *referrer_lyrae_account_ai.unwrap().key,
                market_index as u64,
                &referrer_lyrae_account.perp_accounts[market_index],
                perp_market_cache,
            );
            lyrae_emit!(ReferralFeeAccrualLog {
                lyrae_group: referrer_lyrae_account.lyrae_group,
                referrer_lyrae_account: *referrer_lyrae_account_ai.unwrap().key,
                referree_lyrae_account: *lyrae_account_pk,
                market_index: market_index as u64,
                referral_fee_accrual: ref_fees.to_bits(),
            });
        } else {
            // else user didn't have valid amount of LYR and no valid referrer
            lyrae_account.perp_accounts[market_index].quote_position -= ref_fees;
    ...
    lyrae_account.perp_accounts[market_index].quote_position -= taker_fees;
